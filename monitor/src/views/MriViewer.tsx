...
export default function MriViewer({ id, viewing }: { id: string; viewing: Viewing }) {
  ...
  const [plane, setPlane] = useState(SLICE_TYPE.MULTIPLANAR);
  ...
  useEffect(() => {
    ...
    const urls: string[] = [];
    const nv = new Niivue({ dragAndDropEnabled: false, isNearestInterpolation: true });
    ...
    const load = async () => {
      ...
      if (mask === "compare" && comparison.current) {
        other = new Niivue({ dragAndDropEnabled: false, isNearestInterpolation: true });
        peer.current = other;
    ...
    return () => { disposed = true; controller.abort(); urls.forEach(url => URL.revokeObjectURL(url)); nv.cleanup(); other?.cleanup(); peer.current = null; };
  }, [id, modality, mask]);
  return <section className="mri-viewer" aria-label="MRI outline viewer">
    <div className="viewer-controls">
      ...
      <button disabled={!viewing.outlineCenterMm || loading} onClick={() => { const nv = viewer.current; if (nv && viewing.outlineCenterMm) { nv.scene.crosshairPos = nv.mm2frac(viewing.outlineCenterMm); nv.drawScene(); } }}>Jump to outline</button>
      <button onClick={() => { const nv = viewer.current; if (nv) { nv.scene.crosshairPos = [0.5, 0.5, 0.5]; nv.drawScene(); } }}>Reset position</button>
      {(["X", "Y", "Z"] as const).map((axis, index) => <span key={axis}>{axis} slice <button aria-label={`Previous ${axis} slice`} onClick={() => { const step = [0, 0, 0]; step[index] = -1; viewer.current?.moveCrosshairInVox(step[0], step[1], step[2]); }}>−</button><button aria-label={`Next ${axis} slice`} onClick={() => { const step = [0, 0, 0]; step[index] = 1; viewer.current?.moveCrosshairInVox(step[0], step[1], step[2]); }}>+</button></span>)}
    </div>
