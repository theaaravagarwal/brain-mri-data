...
export function EvidenceView({ resource }: EvidenceViewProps) {
  ...
  const rejected = gate?.decision?.toLowerCase().includes("rejected") ?? false;
  return <section className="evidence-view" aria-labelledby="evidence-title">
    <header className="evidence-header"><div><h1 id="evidence-title">Model evidence</h1><p>Frozen comparison docket · no automatic promotion</p><Freshness resource={resource} /></div><span className={`evidence-gate ${gate?.allowed ? "evidence-gate--open" : "evidence-gate--blocked"}`}>{gate?.allowed ? "Review required" : rejected ? "Candidate rejected" : "Promotion blocked"}</span></header>
    ...
    <section className="evidence-ledger" aria-labelledby="evidence-gate-title"><h2 id="evidence-gate-title">Promotion gate</h2><div className="evidence-gate-copy"><strong>{gate?.allowed ? "Evidence package may proceed to human review" : gate?.decision?.toLowerCase().includes("rejected") ? "Candidate rejected · baseline retained" : "Promotion blocked"}</strong><p>{gate?.decision || "Automatic model promotion remains disabled."}</p></div><ul className="evidence-missing">{gate?.missing?.length ? gate.missing.map(item => <li key={item}>{item}</li>) : <li>No missing evidence reported.</li>}</ul></section>
